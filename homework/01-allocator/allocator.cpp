...
bool AllocatorNode::usage()
{
    return !!(head & flg_mask);
}        

size_t AllocatorNode::length()
{
    return head & ~flg_mask;
}
...
Allocator::Allocator(void* base, size_t size):
    base(base),
    first_node((AllocatorNode*)base),
    last_node(first_node),
    ptr_first((AllocatorNode**)((char*)base + size - size % sizeof(size_t))),
    ptr_last(ptr_first)
{
...
void Allocator::realloc(Pointer& p, size_t N)
{
    ...
    if (node != last_node) { // try expand
        ...
        if (!next->usage()) {
            size_t avail_size = (node->length() + next->length() + 1) * sizeof(size_t);
            if (avail_size >= N) {
                realloc_node(node, N);
                return;
            }
...
void Allocator::free_node(AllocatorNode* node)
{
    ...
    if (node != first_node) {
        ...
        if (!prev->usage()) {
            ...
            if (node == last_node) last_node = prev;
            node = prev;
        }
        
        if (node != last_node && !node->next()->usage()) {
            if (node->next() == last_node) last_node = node;
